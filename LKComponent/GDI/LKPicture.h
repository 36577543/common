...
public:
    CLKPicture(void);
    virtual ~CLKPicture(void);

    bool GetPaintRect(RECT *lpRect){return !!CopyRect(lpRect, &m_PaintRect);}
    bool SetPaintRect(const RECT *lpRect){ return !!CopyRect(&m_PaintRect, lpRect); }
	virtual void UnLoad(); // stops animation plus releases all resources

    bool IsGIF() const{ return m_bIsGIF; }
    bool IsPlaying() const{ return m_bIsPlaying; }
    bool IsAnimatedGIF() const{ return (m_bIsGIF && (m_arrFrames.size() > 1)); }
    SIZE GetSize() const{ return m_PictureSize; }
	int GetFrameCount() const;
    COLORREF GetBkColor() const{ return m_clrBackground; }
    virtual void SetBkColor(COLORREF clr){m_clrBackground = clr;}
...
protected:
    ...
    // 计算图片对应的像素宽度
    virtual void CalcImageWidth(int hmWidth, int hmHeight){m_PictureSize.cx = hmWidth; m_PictureSize.cy = hmHeight; }
};
