...
int CDynDialogEx::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	if (m_pFont == NULL) {
		...
		CString sF = _T("MS Sans Serif");
		memcpy(LogFont.lfFaceName, sF.GetBuffer(), sF.GetLength() * sizeof(TCHAR));
		LogFont.lfHeight = 8;
...
int CDynDialogEx::DoModal() 
{
	...
	int cWC = strFontName.GetLength();
	int nFontNameLen = (cWC) * sizeof(WCHAR);
	...
	//CString strCaption = LogFont.lfFaceName;
	...
	//szBoxCaption[cWC] = 0;
	szBoxLen = m_strCaption.GetLength() * sizeof(WCHAR);
	...
	if (hLocal != NULL) {
		...
		*(WORD*)(pdest + 1) = 0;							// use default window class -- WORD to say it is 0 bytes
		pdest += sizeof(WORD);								// Increment
		memcpy(pdest, m_strCaption.GetBuffer(), szBoxLen);			// Caption
		pdest += szBoxLen;

		*(WORD*)pdest = m_wFontSize;						// font size
		pdest += sizeof(WORD);
		memcpy(pdest, strFontName.GetBuffer(), nFontNameLen);		// font name
		pdest += nFontNameLen;
...
void CDynDialogEx::OnDraw(CDC *pDC, CRect &rtWnd)
{
	...
	if (pImg)
	{
		...
		//pImg->Draw(pDC->GetSafeHdc(), rtWnd);
		pImg->DrawToDC(pDC, rtWnd, 0, LKSKINBASE_DRAWBGSTYLE_ALL, 0, angleOffset);
		//pImg->DrawToDC(pDC, rtWnd, 0, LKSKINBASE_DRAWBGSTYLE_FRAME);
	}
}
...
LRESULT CDynDialogEx::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if (message == WM_LBUTTONDOWN)
	{
		if (m_pParentWnd)
		{
			m_pParentWnd->PostMessage(message, wParam, lParam);
			//m_pParentWnd->PostMessage(WM_LBUTTONUP, wParam, lParam);
		}
		return 0;
	}
